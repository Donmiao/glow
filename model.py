...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    with tf.name_scope('input'):
        X = tf.placeholder(
            # tf.uint8, [hps.image_size, hps.image_size, 3], name='image')
    ...
    def _f_loss(x, y, is_training, reuse=False):
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            if y is not None:
                ...
                y_onehot = tf.cast(tf.one_hot(y[0], hps.n_y, 1, 0), 'float32')

            # x = np.array(x, np.float32).transpose(1, 2, 0)
            # Discrete -> Continuous
            print(type(x))
            if ('numpy.ndarray' in str(type(x)) and x.ndim == 3):  # numpy.ndarray
                # if (len(list(xZ.size())) == 3) :
            ...
            # Generative loss
            nobj = - objective
            bits_x = nobj / (np.log(2.) * int(tf.convert_to_tensor(x).get_shape()[1]) * int(
                tf.convert_to_tensor(x).get_shape()[2]) * int(
                tf.convert_to_tensor(x).get_shape()[3]))  # bits per subpixel
    ...
    def f_loss(iterator, is_training, reuse=False):
        if hps.direct_iterator and iterator is not None:
            ...
            # x, y = np.array(X), np.array(Y)  # X.numpy(), Y.numpy()
            x, y = X, Y
