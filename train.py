...
def get_data(hps, sess):
    ...
    if hps.n_test == -1:
        hps.n_test = {'mnist': 10000, 'cifar10': 10000, 'imagenet-oord': 50000, 'imagenet': 50000,
                      'celeba': 3000, 'lsun_realnvp': 300, 'lsun': 300}[hps.problem]
    ...
    print("Rank {} Batch sizes Train {} Test {} Init {}".format(
        0, hps.local_batch_train, hps.local_batch_test, hps.local_batch_init))
    ...
    if hps.problem in ['imagenet-oord', 'imagenet', 'celeba', 'lsun_realnvp', 'lsun']:
        ...
        train_iterator, test_iterator, data_init = \
            v.get_data(hps.problem, 1, 0, hps.dal, hps.local_batch_train,
...
def main(hps):
    ...
    tf.set_random_seed(1 * hps.seed)
    np.random.seed(1 * hps.seed)
    ...
    #model = model.model(sess, hps, enumerate(train_loader), enumerate(test_loader), data_init)
...
if __name__ == "__main__":
    ...
    # Dataset hyperparams:
    parser.add_argument("--problem", type=str, default= 'imagenet-oord', #cifar10',
